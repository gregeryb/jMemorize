...
public class SimpleInfoDialog extends JDialog {
 ...
 public SimpleInfoDialog(Frame parent, String title, boolean bModal,
  ...
  if (type == QUESTION) {
   buttonLabels = new Object[]{Translatrix.getTranslationString("DialogAccept"),
    Translatrix.getTranslationString("DialogCancel")};
   jOptionPane = new JOptionPane(new JLabel(message, SwingConstants.CENTER),
    JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, null, buttonLabels,
    buttonLabels[0]);
  } else {
   buttonLabels = new Object[]{Translatrix.getTranslationString("DialogClose")};
   jOptionPane = new JOptionPane(new JLabel(message, SwingConstants.CENTER),
    JOptionPane.PLAIN_MESSAGE, JOptionPane.DEFAULT_OPTION, null, buttonLabels,
    buttonLabels[0]);
  }
  ...
  jOptionPane.addPropertyChangeListener(new PropertyChangeListener() {
   public void propertyChange(PropertyChangeEvent e) {
    ...
    if (isVisible() && (e.getSource() == jOptionPane) && (prop.equals(
  ...
  this.pack();
  int centerX = (int) (((parent.getSize().getWidth() / 2) + parent.getLocation()
   .getX()) - (this.getSize().getWidth() / 2));
  int centerY = (int) (((parent.getSize().getHeight() / 2) + parent
   .getLocation().getY()) - (this.getSize().getHeight() / 2));
  if (centerX < 0) {
   centerX = 0;
  }
  if (centerY < 0) {
