...
class ImageFileChooserPreview extends JComponent implements
 ...
 public void loadImage() {
  ...
  if (imageThumb.getIconHeight() < previewHeight && imageThumb.getIconWidth()
   < previewWidth) {
   return;
  }
  int w = previewWidth;
  ...
  imageThumb = new ImageIcon(imageThumb.getImage().getScaledInstance(w, h,
   Image.SCALE_DEFAULT));
 }

 /**
  * Callback (event handler) to indicate that a property of the JFileChooser has
  * changed. If the selected file has changed cause a new thumbnail to load.
  */
