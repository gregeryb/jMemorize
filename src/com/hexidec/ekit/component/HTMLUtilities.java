...
public class HTMLUtilities {
 ...
 public HTMLUtilities(EkitCore newParent) {
   ...
   * classTag.getClass().getDeclaredFields(); for(int i = 0; i < fields.length;
   * i++) { try { System.out.println("Adding " +
 ...
 /**
  * Diese Methode gibt jeweils den Start- und Endoffset des Elements sowie dem
  * entsprechenden schlie�enden Tag zur�ck
  */
 public int[] getPositions(Element element, String source, boolean closingTag,
  String idString) {
  HTML.Tag tag = getHTMLTag(element);
 ...
 public boolean checkParentsTag(HTML.Tag tag) {
  Element e = parent.getHTMLDoc().getParagraphElement(parent.getCaretPosition());
 ...
 /*
  * Diese Methoden geben das erste gefundende dem �bergebenen tags entsprechende
  * Element zur�ck
  */
 public Element getListItemParent() {
  String listItemTag = HTML.Tag.LI.toString();
  Element eleSearch = parent.getHTMLDoc().getCharacterElement(parent
   .getCaretPosition());
  do {
 ...
 public Element getListItemContainer() {
  ...
  Element eleSearch = parent.getHTMLDoc().getCharacterElement(parent
   .getCaretPosition());
  do {
   if (listUnorderedTag.equals(eleSearch.getName()) || listOrderedTag.equals(
    eleSearch.getName())) {
    return eleSearch;
   }
   eleSearch = eleSearch.getParentElement();
 ...
 public SimpleAttributeSet removeAttribute(SimpleAttributeSet sourceAS,
  ...
  try {
   ...
   for (int countSource = 0; countSource < sourceKeys.length; countSource++) {
    hit = false;
    if (sourceKeys[countSource] == "name" | sourceKeys[countSource]
     == "resolver") {
     hit = true;
    } else {
 ...
 public void delete()
  ...
  htmlString += jtpSource.getText().substring(start + posStrings[0].length(),
   end);
  htmlString += jtpSource.getText().substring(end + posStrings[1].length(),
   jtpSource.getText().length());
