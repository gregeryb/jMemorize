 ...
 * Copyright (c) 1991-2004 Unicode, Inc. All rights reserved. Distributed under the Terms of Use in http://www.unicode.org/copyright.html.
 */
...
public class UnicodeDialog extends JDialog implements ActionListener {
 ...
 private static final String[] unicodeBlocks
  = {
   ...
   "Greek and Coptic", "Cyrillic", "Cyrillic Supplement", "Armenian", "Hebrew",
   "Arabic", "Syriac", "Thaana", "Devanagari", "Bengali", "Gurmukhi", "Gujarati",
   "Oriya", "Tamil", "Telugu", "Kannada", "Malayalam", "Sinhala", "Thai", "Lao",
   "Tibetan", "Myanmar", "Georgian", "Hanjul Jamo", "Ethiopic", "Cherokee",
   "Unified Canadian Aboriginal Syllabics", "Ogham", "Runic", "Tagalog",
   "Hanunoo", "Buhid", "Tagbanwa", "Khmer", "Mongolian", "Limbu", "Tai Le",
   "Khmer Symbols", "Phonetic Extensions", "Latin Extended Additional",
   "Greek Extended", "Punctuation / Scripts / Currency / Diacriticals",
  ...
  * 8400-8447 "Letterlike Symbols / Number Forms / Arrows" "Letterlike Symbols"
  * -> 8448-8527 "Number Forms"	-> 8528-8591 "Arrows"	-> 8592-8703 "Box Drawing
  * / Block Elements / Geometric Shapes" "Box Drawing" -> 9472-9599 "Block
  * Elements"	-> 9600-9631 "Geometric Shapes"	-> 9632-9727 "Dingbats / Math-A /
  * Arrows-A" "Dingbats"	-> 9984-10175 "Miscellaneous Mathematical Symbols-A"	->
  * 10176-10223 "Supplemental Arrows-A"	-> 10224-10239 "Arrows-B / Math-B"
  * "Supplemental Arrows-B"	-> 10496-10623 "Miscellaneous Mathematical
  * Symbols-B"	-> 10624-10751
  */
  ...
  * "Gothic"	-> 66352-66383 "Ugaritic"	-> 66432-66463 "Deseret"	-> 66560-66639
  * "Shavian"	-> 66640-66687 "Osmanya"	-> 66688-66735 "Cypriot Syllabary"	->
  * 67584-67647 "Byzantine Musical Symbols"	-> 118784-119039 "Musical Symbols"
  * -> 119040-119295 "Tai Xuan Jing Symbols"	-> 119552-119647 "Mathematical
  * Alphanumeric Symbols"	-> 119808-120831 "CJK Unified Ideographic Extension B"
  * -> 131072-173791 "CJK Compatibility Ideographs Supplement"	-> 194560-195103
  * "Tags"	-> 917504-917631 "Variations Selector Supplement"	-> 917760-917999
  * "Supplementary Private Use Area-A"	-> 983040-1048573 "Supplementary Private
  * Use Area-B"	-> 1048576-1114109
  */
 ...
 public void actionPerformed(ActionEvent ae) {
  if (ae.getActionCommand().equals(CMDCHANGEBLOCK)) {
   populateButtons(jcmbBlockSelector.getSelectedIndex(), jcmbPageSelector
    .getSelectedIndex());
   ...
   try {
    ...
    System.out.println("IOException during character insertion : " + ioe
     .getMessage());
   } catch (javax.swing.text.BadLocationException ble) {
    System.out.println("BadLocationException during character insertion : "
     + ble.getMessage());
   }
  }
 }
 ...
 public void init(int startIndex) {
  String customFont = Translatrix
   .getTranslationString("UnicodeDialogButtonFont");
  if (customFont != null && customFont.length() > 0) {
   buttonFont = new Font(Translatrix.getTranslationString(
    "UnicodeDialogButtonFont"), Font.PLAIN, 12);
  } else {
  ...
  for (int labelLoop = 0; labelLoop < 16; labelLoop++) {
   JLabel jlblMarker = new JLabel("x" + (labelLoop > 9 ? "" + (char) (65
    + (labelLoop - 10)) : "" + labelLoop));
  ...
  for (int counter = 0; counter < UNICODEBLOCKSIZE; counter++) {
   if ((counter % 16) == 0) {
    JLabel jlblMarker = new JLabel((labelcount > 9 ? "" + (char) (65
     + (labelcount - 10)) : "" + labelcount) + "x");
    jlblMarker.setHorizontalAlignment(SwingConstants.CENTER);
    jlblMarker.setVerticalAlignment(SwingConstants.CENTER);
    jlblMarker.setForeground(new Color(0.5f, 0.5f, 0.75f));
    centerPanel.add(jlblMarker);
   ...
   buttonArray[counter].setBorder(javax.swing.BorderFactory.createEtchedBorder(
    javax.swing.border.EtchedBorder.LOWERED));
   buttonArray[counter].addActionListener(this);
   if (counter == 0) {
    FontRenderContext frcLocal = ((java.awt.Graphics2D) (parentEkit
     .getGraphics())).getFontRenderContext();
    Rectangle2D fontBounds = buttonFont.getMaxCharBounds(frcLocal);
    int maxCharWidth = (int) (Math.abs(fontBounds.getX())) + (int) (Math.abs(
     fontBounds.getWidth()));
    int maxCharHeight = (int) (Math.abs(fontBounds.getY())) + (int) (Math.abs(
     fontBounds.getHeight()));
    Insets buttonInsets = buttonArray[counter].getBorder().getBorderInsets(
     buttonArray[counter]);
   ...
   buttonArray[counter].setPreferredSize(new Dimension(prefButtonWidth,
    prefButtonHeight));
  ...
  selectorPanel.add(new JLabel(Translatrix.getTranslationString(
   "SelectorToolUnicodeBlock")));
  selectorPanel.add(jcmbBlockSelector);
  selectorPanel.add(new JLabel(Translatrix.getTranslationString(
   "SelectorToolUnicodePage")));
  selectorPanel.add(jcmbPageSelector);
  JPanel buttonPanel = new JPanel();
  JButton closeButton = new JButton(Translatrix.getTranslationString(
   "DialogClose"));
 ...
 private void populateButtons(int index, int page) {
  ...
  int firstInt = ((unicodeBlockStart[index] / UNICODEBLOCKSIZE)
   * UNICODEBLOCKSIZE) + (page * UNICODEBLOCKSIZE);
