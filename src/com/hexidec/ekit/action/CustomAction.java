  ...
  if (this.isEnabled()) {
   ...
   if (selText == null || textLength < 1) {
    SimpleInfoDialog sidWarn = new SimpleInfoDialog(parentEkit.getFrame(),
     ...
     .getTranslationString("ErrorNoTextSelected"), SimpleInfoDialog.ERROR);
   } else {
    ...
    if (htmlTag.toString().equals(HTML.Tag.A.toString())) {
     if (!htmlAttribs.containsKey("href")) {
      UserInputAnchorDialog uidInput = new UserInputAnchorDialog(parentEkit,
     ...
     if (htmlAttribs.containsKey("color")) {
      Color color = new JColorChooser().showDialog(parentEkit.getFrame(),
       Translatrix.getTranslationString("CustomColorDialog"), Color.black);
      if (color != null) {
       StyledEditorKit.ForegroundAction customColorAction = new StyledEditorKit.ForegroundAction(
        "CustomColor", color);
    ...
    if (htmlAttribs2.size() > 0) {
     ...
     SimpleAttributeSet baseAttrs = new SimpleAttributeSet(parentEkit
      .getTextPane().getCharacterAttributes());
