  ...
  try {
   ...
   if (selTextBase == null || textLength < 1) {
    ...
    if (ae.getActionCommand() != "newListPoint") {
     if (htmlUtilities.checkParentsTag(HTML.Tag.OL) || htmlUtilities
      .checkParentsTag(HTML.Tag.UL)) {
    ...
    if (htmlUtilities.checkParentsTag(baseTag)) {
     sbNew.append("<li></li>");
     insertHTML(parentEkit.getTextPane(), parentEkit.getHTMLDoc(), parentEkit
      .getTextPane().getCaretPosition(), sbNew.toString(), 0, 0, HTML.Tag.LI);
    } else {
     boolean isLast = false;
     int caretPos = parentEkit.getCaretPosition();
     if (caretPos == parentEkit.getHTMLDoc().getLength()) {
      isLast = true;
     }
     sbNew.append("<" + sListType + "><li></li></" + sListType + ">"
      + (isLast ? "<p style=\"margin-top: 0\">&nbsp;</p>" : ""));
     insertHTML(parentEkit.getTextPane(), parentEkit.getHTMLDoc(), parentEkit
      .getTextPane().getCaretPosition(), sbNew.toString(), 0, 0, (sListType
    ...
    if (htmlUtilities.checkParentsTag(HTML.Tag.OL) || htmlUtilities
     ...
     /*
      * for(int ch = 0; ch < selText.length(); ch++) { Element elem =
      * htmlDoc.getCharacterElement(iStart + ch); System.out.println("elem " +
      * ch + ": " + elem.getName()); System.out.println("char " + ch + ": " +
      * selText.charAt(ch) + " [" +
      * Character.getNumericValue(selText.charAt(ch)) + "]");
      * if(Character.getNumericValue(selText.charAt(ch)) < 0) {
      * System.out.println(" is space? " + ((selText.charAt(ch) == '\u0020') ?
      * "YES" : "---")); System.out.println(" is return? " +
      * ((selText.charAt(ch) == '\r') ? "YES" : "---")); System.out.println(" is
      * newline? " + ((selText.charAt(ch) == '\n') ? "YES" : "---"));
      * System.out.println(" is nextline? " + ((selText.charAt(ch) == '\u0085')
      * ? "YES" : "---")); System.out.println(" is linesep? " +
      * ((selText.charAt(ch) == '\u2028') ? "YES" : "---"));
      * System.out.println(" is parasep? " + ((selText.charAt(ch) == '\u2029') ?
      * "YES" : "---")); System.out.println(" is verttab? " +
      * ((selText.charAt(ch) == '\u000B') ? "YES" : "---"));
      * System.out.println(" is formfeed? " + ((selText.charAt(ch) == '\u000C')
      * ? "YES" : "---")); } }
      */
     StringBuffer sbNew = new StringBuffer();
...
//System.out.println("Reverting list " + element.toString());
  ...
  parentEkit.getHTMLDoc().replaceAttributes(element, sa, tag);
  parentEkit.refreshOnUpdate();
