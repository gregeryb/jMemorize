...
//  Processor Methods -----------------------------------------------------------------------/
 public static String encode(byte[] sourceBytes) {
  ...
  while (bytePlace < sourceBytes.length) {
   tokenValue = (byte) ((byte) tokenValue | (byte) ((sourceBytes[bytePlace] & (1
    << bitOffset)) > 0 ? (1 << b64Offset) : (byte) 0));
   bitOffset--;
   if (bitOffset < 0) {
    bitOffset = 7;
    bytePlace++;
  ...
  if (b64Offset != 5) {
   ...
   for (int i = b64Offset; i >= 0; i--) {
    if (bitOffset >= 0) {
     tokenValue = (byte) ((byte) tokenValue | (byte) ((sourceBytes[bytePlace]
      & (1 << bitOffset)) > 0 ? (1 << i) : (byte) 0));
    }
    bitOffset--;
   }
   encoding.append(Base64Tokens.elementAt(tokenValue));
 ...
 public static byte[] decodeBytes(byte[] source) {
  ...
  while (bytePlace < source.length) {
   ...
   if ((char) (source[bytePlace]) == LINEFEED || (char) (source[bytePlace])
    == CARRIAGE) {
    ...
    if (!Base64Tokens.contains("" + (char) (source[bytePlace]))) {
     ...
     charValue = (byte) ((byte) charValue | (byte) ((currentByte & (1
      << b64Offset)) > 0 ? (1 << bitOffset) : (byte) 0));
     bitOffset--;
     if (bitOffset < 0) {
      bitOffset = 7;
