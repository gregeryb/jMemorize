...
public class CsvBuilder {
  ...
  *
  * @param delimiter the delimiter that is used to separate values in the file.
 ...
 public static void importLesson(InputStream in, char delimiter,
  ...
  try {
   ...
   while (reader.readRecord()) {
    String frontSide = reader.get(FRONTSIDE_COL);
    String flipSide = reader.get(FLISIDE_COL);
    if (frontSide.isEmpty() || flipSide.isEmpty()) {
    ...
    if (categoryName.length() == 0 || categoryName.equalsIgnoreCase(rootCategory
     .getName())) {
 ...
 private static void validateHeader(String[] headers) throws BadHeaderException {
  ...
  for (int i = 0; i < headers.length; i++) {
   ...
   if (!headers[i].equalsIgnoreCase(CATEGORY_COL) && !headers[i]
    .equalsIgnoreCase(LEVEL_COL)) {
