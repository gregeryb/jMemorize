...
public class PdfRtfBuilder {
  ...
  */
 public static void exportLessonToPDF(Lesson lesson, File file) throws
  IOException {
  export(lesson, PDF_MODE, file);
 }

 public static void exportLessonToRTF(Lesson lesson, File file) throws
  IOException {
  export(lesson, RTF_MODE, file);
 }

 private static void export(Lesson lesson, int mode, File file) throws
  IOException {
  FontFactory.registerDirectories();
  // set up the fonts we will use to write the front and back of cards
  String frontFontName = Settings.loadFont(FontType.CARD_FRONT).getFont()
   .getFamily();
  frontFont = FontFactory.getFont(frontFontName, BaseFont.IDENTITY_H,
   BaseFont.EMBEDDED);
  String backFontName = Settings.loadFont(FontType.CARD_FLIP).getFont()
   .getFamily();
 ...
 private static void writeCard(Document doc, Card card)
  ...
  table.complete();
  Phrase front = new Phrase(card.getFrontSide().getUnformattedText(), frontFont);
  table.addCell(front);
  Phrase back = new Phrase(card.getBackSide().getUnformattedText(), backFont);
  table.addCell(back);
  doc.add(table);
 }
