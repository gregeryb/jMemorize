 ...
 * boolean)} or {@link #cardSkipped()}. This makes the learn session perform
 * some action on the card.</li>
...
public class DefaultLearnSession implements CategoryObserver, LearnSession {
 ...
 /**
  * A Comparator that is used for sorting cards in learn sessions. This is used
  * to sort the cards into equivalence classes, from which the next card will be
  * drawn randomly.
  */
...
//            return (date0.before(date1) ? -1 : 1);
 ...
 /**
  * This class is a wrapper for a card. It allows to associate additional data
  * to a card, that is only relevant during a single specifc learn session.
  */
 private class CardInfo {

  private Card m_card;
  /**
   * For learning this variable should be used instead of the real level of the
   * card. This allows for some special shuffling techniques.
   */
  private int m_level;
 ...
 public DefaultLearnSession(Category category,
  ...
  m_cardsReserve = new EquivalenceClassSet<CardInfo>(m_cardsActive
   .getComparator());
 ...
 /*
  * (non-Javadoc) @see jmemorize.core.LearnSession
  */
 public void startLearning() {
  ...
  if (m_settings.isCardLimitEnabled() && m_cardsActive.size() > m_settings
   .getCardLimit()) {
 ...
 public void cardChecked(boolean passed, boolean shownFlipped) {
  ...
  if (passed) {
    ...
    * equivalence class (i.e. the set is in an inconsistent state). We can't fix
    * it until after the reset, *but* the resetCardLevel() method fires the
    * event which results in the observers reacting (checking for end of
    * session, getting the next card, etc). The card's equivalence class will be
    * wrong, but this should not be a problem for gotoNextCard. We reset the
    * equivalence class as soon as possible.
    */
 ...
 public void cardSkipped() {
  ...
  if (m_cardsReserve != null && m_cardsReserve.size() > 0) {
   ...
   if (replacementCard.getLearnedAmount(true) > 0 || replacementCard
    .getLearnedAmount(false) > 0) {
 ...
 public boolean isQuit() {
  ...
  boolean limitReached = m_settings.isCardLimitEnabled() && m_cardsLearned
   .size() >= m_settings.getCardLimit();
  ...
  *
  * @return <code>true</code> if the card should be flipped. <code>false</code>
  * otherwise.
  */
 private boolean checkIfFlipped() {
  if (m_settings.getSidesMode() == LearnSettings.SIDES_RANDOM) {
   ...
   int timesToLearnFront
    = m_settings.getAmountToTest(true) - currentCard.getLearnedAmount(true);
   int timesToLearnBack
    = m_settings.getAmountToTest(false) - currentCard.getLearnedAmount(false);
 ...
 /**
  * @return Cards that are being learned can be grouped by categories. In this
  * case the map holds for every category the position when it should appear.
  */
