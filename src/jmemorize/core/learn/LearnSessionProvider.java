 ...
 *
 * Use this interface by calling <code>Main.getInstance()</code>. {@link Main}
...
public interface LearnSessionProvider {
  ...
  * @param the cards that were selected in the main card table when starting the
  * learn session (these will be learned if learnUnlearned and learnExpired are
  * false).
  * @param category the base category that holds the cards for this session.
  * @param learnUnlearned <code>true</code> if unlearned cards should appear in
  * this session. <code>false</code> otherwise.
  * @param learnExpired <code>true</code> if expired cards should appear in this
 ...
 /**
  * This method should only be called by the learn session itself to notify its
  * provider to notify all observers about the session end.
  */
 public void sessionEnded(LearnSession session);

 /**
  * @return <code>true</code> if there is currently are running learn session.
