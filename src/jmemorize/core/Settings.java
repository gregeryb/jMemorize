...
public class Settings {
 ...
 /**
  * Listens for changes to the currently set card font which can be set by using
 ...
 /**
  * @return The locale stored in preferences or the default locale if no locale
  * was stored.
  */
...
//            Main.logThrowable("failed to load font alignment.", e);
 ...
 public static void storeStrategy(LearnSettings strategy) {
  ...
  PREFS.putBoolean(SCHEDULE_FIXED_EXPIRATION_ENABLED, strategy
   .isFixedExpirationTimeEnabled());
  PREFS
   .putInt(SCHEDULE_FIXED_EXPIRATION_HOUR, strategy.getFixedExpirationHour());
  PREFS.putInt(SCHEDULE_FIXED_EXPIRATION_MINUTE, strategy
   .getFixedExpirationMinute());
  PREFS.putBoolean(GROUP_BY_CATEGORY, strategy.isGroupByCategory());
  PREFS.putInt(CATEGORY_ORDER, strategy.getCategoryOrder());
 }

 /**
  * @return A newly created strategy that got instantiated with all the stored
  * strategy data or with default values.
  */
  ...
  *
  * @return <code>true</code> if the main frame should be displayed as maximized
  * at program start.
  */
  ...
  * @param frameId an identifier for the frame.
  *
  * @return the position of the main frame. <code>null</code> if no position is
  * stored.
  */
