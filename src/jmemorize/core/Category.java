...
public class Category implements Events {
 ...
 /**
  * Moves the card to a new category, preserving all its fields and its level.
  * This is different from removing and then adding a card because it triggers a
  * single MOVE event instead of a REMOVE and ADD event.
  */
  ...
  * Fires a DECK_EVENT.
  */
 public static void raiseCardLevel(Card card, Date testDate,
 ...
 /**
  * Removes the card from its current deck and appends it to deck 0 (even if its
  * already at level 0). TotalTests is increased by one.
  *
  * Fires a DECK_EVENT.
  */
 ...
 public static void reappendCard(Card card) {
  ...
  card.getCategory().fireCardEvent(DECK_EVENT, card, card.getCategory(), card
   .getLevel());
 }
  ...
  * directly belong to this category and not to any of this child categories.
  */
 ...
 /**
  * @return All cards in the level that are local to this category. That is all
  * cards that directly belong to this category and not to any of this child
  * categories.
  */
 ...
 /**
  * @return The number of decks of this category and its child categories. That
  * means that no child categoriy can have more number of decks then its parent
  * category.
  */
 ...
 public Category cloneWithoutProgress() {
  ...
  for (Category childCategory : getChildCategories()) {
   clonedCategory.addCategoryChild((Category) childCategory
    .cloneWithoutProgress());
  }
 ...
 void fireCardEvent(int type, Card card, Category category, int deck) {
  ...
  List<CategoryObserver> observersCopy = new ArrayList<CategoryObserver>(
   m_observers);
