...
public class EquivalenceClassSet<T> extends AbstractSet<T> {
 ...
 /*
  * Class invariants: Individual equivalence classes may not be empty The cached
  * size is equal to the sum of all items in the sublists The changeID
  * increments montonically whenever the set contents are changed, although it
 ...
 protected class OnePassIterator implements Iterator<T> {
  ...
  public boolean hasNext() {
   ...
   if (localEqvClassIter.hasNext() || (localItemIter != null && localItemIter
    .hasNext())) {
    return true;
   }
   return false;
 ...
 /**
  * The loopIterator is a special iteration for which hasNext() returns true if
  * the size is greater than 0. The next() method traverses the equivalence
  * classes in comparator order. Within each equivalence class, the items are
  * returned randomly (by shuffling the elements in the equivalence class every
  * time that equivalence class is reached during iteration).
  *
  * Iteration can be reset to the first equivalence class by using the
  * resetLoopIterator method of the main class.
  *
  * @return the iterator
  */
  ...
  * the elements in each equivalence class every time that equivalence class is
 ...
 /*
  * (non-Javadoc) @see java.util.AbstractCollection#size()
  */
 public int size() {
  return m_size;
 }
 ...
 /*
  * Adds the argument to the collection. If the element is not already a member
  * of the set, and is equivalent to the current equivalence class, it is added
  * so that it will be returned before any element from another class is
  * returned.
  */
  ...
  * Adds the argument to the collection. If the element is not already a member
  * of the set, and is equivalent to the current equivalence class, it is added
  * so that it will not be returned before an element from another class is
 ...
 /*
  * (non-Javadoc) @see java.util.AbstractCollection#clear()
  */
 public void clear() {
 ...
 /*
  * (non-Javadoc) @see java.util.AbstractCollection#contains(java.lang.Object)
  */
 public boolean contains(Object arg0) {
  EqvPosition eqvPosition = findEqvClass(arg0);
  if (eqvPosition.matchingEqvClass != null && eqvPosition.matchingEqvClass
   .contains(arg0)) {
 ...
 /*
  * (non-Javadoc) @see java.util.AbstractCollection#remove(java.lang.Object)
  */
  ...
  *
  * The argument specifies how many elements to put in the new set. Elements
  * will be chosen in comparator order. All elements put into the new set will
  * be removed from this set. If the original set contained less elements than
  * the argument, then after the partition the new set will contain all the
  * elements and the original set will be empty.
  *
  * If the partition is non-trivial (that is, if the new set contains at least
  * one element), then the counters for the loop iterator will be reset.
  *
 ...
 /**
  * In some cases, the equivalence class of an object will change. This can
  * leave the list in an inconsistent state. It is essential to fix the problem.
