...
public class CardTable extends JTable implements Settings.CardFontObserver,
 ...
 /**
  * A TableCellRenderer that displays cells with Strings and a specific string
  * suffix.
  */
 ...
 private class CardComparator implements Comparator<Card> {
  ...
  public int compare(Card arg0, Card arg1) {
   Object obj0 = getSortingValue(arg0, m_modelIndex);
   Object obj1 = getSortingValue(arg1, m_modelIndex);
   Comparable col0;
   Comparable col1;
   if (obj0 instanceof String && obj1 instanceof String) {
    col0 = (Comparable) normalize((CharSequence) obj0, Normalizer.Form.NFC);
    col1 = (Comparable) normalize((CharSequence) obj1, Normalizer.Form.NFC);
   } else {
    col0 = (Comparable) obj0;
 ...
 public class ViewModel extends AbstractTableModel {
  ...
  /**
   * Set the model column index and direction by which this table should be
   * sorted.
   *
  ...
  public Object getValueAt(int rowIndex, int columnIndex) {
   ...
   return getDisplayValue(card, columnIndex);
  }
  ...
  * @param defaultColumns if columns can't be loaded from preferences, display
  * these columns.
  */
 ...
 /*
  * (non-Javadoc) @see jmemorize.core.Main.ProgramEndObserver
  */
 ...
 /**
  * Hook the card context menu to given JComponent. Attaches a MouseListener to
  * the given component which checks for right mouse button clicks and shows the
  * context menu.
  */
 ...
 /*
  * (non-Javadoc) @see javax.swing.JTable#prepareRenderer
  */
  ...
  */
 private static Object getDisplayValue(Card card, int modelIndex) {
  switch (modelIndex) {
   case COLUMN_FRONTSIDE:
    return card.getFrontSide().getRowText();
   case COLUMN_BACKSIDE:
    return card.getBackSide().getRowText();
   case COLUMN_DECK:
  ...
  * @return the value at given model index for given card.
  */
 private static Object getSortingValue(Card card, int modelIndex) {
  switch (modelIndex) {
   case COLUMN_FRONTSIDE:
    return card.getFrontSide().getUnformattedText().toUpperCase().replaceAll(
     "\\(.*?\\)", "").trim();
   case COLUMN_BACKSIDE:
    return card.getBackSide().getUnformattedText().toUpperCase().replaceAll(
     "\\(.*?\\)", "").trim();
   case COLUMN_DECK:
    return card.getLevel();
   case COLUMN_CATEGORY:
    return card.getCategory().getName().toUpperCase();
   case COLUMN_PATH:
    return card.getCategory().getPath().toUpperCase();
   case COLUMN_CREATED:
    return card.getDateCreated();
   case COLUMN_MODIFIED:
    return card.getDateModified();
 ...
 private void loadFromPreferences() {
  ...
  if (columns == null || columns.length <= 0 || columns.length
   >= COLUMN_NAMES.length) {
