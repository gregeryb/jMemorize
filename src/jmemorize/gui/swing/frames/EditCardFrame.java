...
public class EditCardFrame extends EscapableFrame implements CategoryObserver,
  ...
  * @param card the card that is to be shown and editted.
  *
  * @param cards the cards that belong to the context of the card that is to be
  * edited. These cards are used to allow browsing to next and previous card.
  * Therefore the card given on the former parameter is usually also part of
  * this list. The usual mode is to show the currently selected card and to give
  * all other cards that are part of the same card table/learn history etc. as
  * additional cards.
  *
  * @param category The category that includes all cards from former parameters.
  */
  ...
  * entries, this will bring up a dialog that asks if the user wants to save the
  * changes. If yes is selected the card sides are saved.
  *
 ...
 private boolean isChanged() {
  boolean categoryChanged = m_currentCard.getCategory() != m_cardPanel
   .getCategoryComboBox().getSelectedCategory();
  if (categoryChanged) {
   return true;
  }
  ...
  boolean textChanged
   = !m_cardPanel.getFrontText().equals(frontSide.getText()) || !m_cardPanel
   .getBackText().equals(backSide.getText());
  if (textChanged) {
   return true;
  }
  return false;
 ...
 private void updateTitle() {
  // set title
  String title = m_cardPanel.getFrontUnformattedText();
  title = title.replace('\n', ' ');
 ...
 private boolean saveCard() {
  if (m_cardPanel.isValidCard()) {
   String frontText = m_cardPanel.getFrontText();
   String backText = m_cardPanel.getBackText();
