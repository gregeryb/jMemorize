...
public class NewCardFrame extends EscapableFrame {
 ...
 public NewCardFrame(NewCardFramesManager manager, Category currentCategory) {
  ...
  categoryComboBox.setRootCategory(Main.getInstance().getLesson()
   .getRootCategory());
 ...
 public boolean close() {
  if (m_cardPanel.getFrontText().trim().length() > 0 || m_cardPanel
   .getBackText().trim().length() > 0) {
 ...
 private boolean createCard(boolean keepOpen) {
  if (m_cardPanel.isValidCard()) {
   String frontside = m_cardPanel.getFrontText();
   String backside = m_cardPanel.getBackText();
