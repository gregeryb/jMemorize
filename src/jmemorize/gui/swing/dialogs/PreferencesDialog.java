...
public class PreferencesDialog extends JDialog {
 ...
 public PreferencesDialog(MainFrame frame) {
  ...
  m_langComboBox
   .setModel(new DefaultComboBoxModel(formatLocaleStrings(locales)));
...
//        setSize(400, getHeight());
 ...
 private JPanel buildFontPanel() {
  ...
  previewPanel.setBorder(new TitledBorder(Localization.get(
   LC.PREFERENCES_PREVIEW)));
  // build panel
  FormLayout layout = new FormLayout(
  ...
  DefaultFormBuilder builder = new DefaultFormBuilder(layout);
  builder.setDefaultDialogBorder();
  builder.addSeparator(Localization.get(LC.PREFERENCES_FONT_SETTINGS), cc.xyw(1,
   1, 5));
 ...
 private Locale getSelectedLocale() {
  return (Locale) Localization.getAvailableLocales().get(m_langComboBox
   .getSelectedIndex());
 }
 ...
 private void updateListFromFont() {
  ...
  m_sizeList.setSelectedValue(Integer.toString(cardFont.getFont().getSize()),
   true);
 ...
 private void updateFontPreview() {
  ...
  switch (cardFont.getAlignment()) {
   case CENTER: hAlign = SwingConstants.CENTER;
    break;
   case RIGHT: hAlign = SwingConstants.TRAILING;
    break;
  }
