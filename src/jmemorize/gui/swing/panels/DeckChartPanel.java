...
public class DeckChartPanel extends JPanel implements CategoryObserver {
 ...
 private class MouseClicked implements ChartMouseListener {
  ...
  /*
   * (non-Javadoc) @see org.jfree.chart.ChartMouseListener
   */
...
//            domainAxis.setCategoryMargin(0.2 + (0.011 * getMinNumDecks()));
  ...
  /*
   * NOTE this is a workaround. JFreeChart appears to insist on drawing bars
   * even if they are of zero width. To prevent the top of the stacked bar chart
   * from being the wrong color, we need to skip bars that are zero width. Here,
   * we intercept the draw call to remove any zero width blocks in the stack
   * before drawing
   */
...
//        legend.setsetRenderingOrder(LegendRenderingOrder.REVERSE);
 ...
 /**
  * Sets the values for the column with given id. This method also handles the
  * order in which the rows will appear in the column.
  */
