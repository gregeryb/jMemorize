...
public class CardCounterPanel extends JPanel {
 ...
 public void setCardsPassed(int passed, int partiallyPassed) {
  ...
  if (m_cardsTarget > -1) {
   if (USE_PIECHART) {
    int cardsUnlearned = m_cardsTarget
 ...
 private ChartPanel buildPiePanel() {
   ...
   * causes an affine transform to be applied to charts smaller than 300x200. If
   * the minimum sizes are not set to the same value, the chart and legend are
   * scaled by the proprtion! see
 ...
 private ExtentProgressBar buildExtentProgressBar() {
  ...
  Color transparentGreen = new Color(grn.getRed(), grn.getGreen(), grn.getBlue(),
   128);
 ...
 private float[] getValues() {
  ...
  for (int i = 0; i < m_cards.size(); i++) {
   ...
   if (m_session.getPassedCards().contains(card) || m_session
    .getRelearnedCards().contains(card)) {
    vals[i] = 1f;
   } else {
