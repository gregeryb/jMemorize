...
public class SessionChartPanel extends JPanel {
 ...
 private void initComponents(LearnSession finishedSession) {
  ...
  add(createChartPanel(Localization.get(LC.CHART_AVERAGE_SESSION), history
   .getAverage()));
 ...
 private JFreeChart createChart(String title, SessionSummary summary) {
  ...
  plot.setSectionPaint(Localization.get(LC.LEARNED),
   ColorConstants.LEARNED_CARDS);
  plot
   .setSectionPaint(Localization.get(LC.FAILED), ColorConstants.EXPIRED_CARDS);
  plot.setSectionPaint(Localization.get(LC.SKIPPED),
   ColorConstants.UNLEARNED_CARDS);
  plot.setSectionPaint(Localization.get(LC.RELEARNED),
   ColorConstants.RELEARNED_CARDS);
  return chart;
 }
